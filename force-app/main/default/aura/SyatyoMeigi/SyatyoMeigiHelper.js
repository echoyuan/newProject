({
    ...
    upsertHandler: function(component, event, helper) {
        ...
        this.savePj(component, pj, function(response){
            ...
            if (state == "SUCCESS") {
                ...
                var title = "氏名 \""+ cmpView.recordInfo.SHMI__c + "\" が" + acctionStr + "されました。";
    ...
    checkHandler: function(component, event, helper) {
        var pj = component.get("v.recordInfo");
        if (pj.SHMI__c == null || pj.SHMI__c == '') {
            this.handleShowToast(component, "氏名を有効な値に選択しなければ、実行できません。", "error");
            return;
        }
        var errflg = false;
