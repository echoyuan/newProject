({
    // 保存処理
    savePj: function(component, KAISYA_KANRI__c, callback) {
    ...
    upsertHandler: function(component, event, helper) {
        ...
        this.savePj(component, pj, function(response){
            ...
            if (state == "SUCCESS") {
                ...
                debugger;
                var title = "会社名 \""+ cmpView.recordInfo.KAISYAMEI_c + "\" が" + acctionStr + "されました。";
    ...
    checkHandler: function(component, event, helper) {
        var pj = component.get("v.recordInfo");
        if (pj.KAISYAMEI__c == null || pj.KAISYAMEI__c == '') {
            this.handleShowToast(component, "会社名を有効な値に選択しなければ、実行できません。", "error");
           return;
        }
        if (pj.SYATYO_MEIGI__c == null || pj.SYATYO_MEIGI__c == '') {
           this.handleShowToast(component, "社長名義を有効な値に選択しなければ、実行できません。", "error");
           return;
        }
        debugger;
        var errflg = false;
